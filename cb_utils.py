...
def peak_counter(df, jelly, thresh, fps, lam_big, lam_small, data_start, data_end, n):
    ...
    Input:
        ...
        lam_big: filtering parameter for baseline
    ...
    # if data, then continue   
        ...
        # print(trace) gives you 4799 values from jellyfish 0
        # Array of frames in the file
        frames = np.array(range(len(df[jelly][data_start:data_end])))
        # print(frames) gives you total amount of frames
        # Convert frames to time in seconds 
        time = frames * 1.0 / (fps) # time in s

        # Find the baseline by smoothing the trace
        # uncomment
        baseline = jb.nw_kernel_smooth(frames, frames, trace, jb.epan_kernel, lam_small)

        # Find the max based on the delta from baseline to max height of the pulse
        # from a section of the trace

        ################################################
        ######## Try minimum for inverted threshold ####
        ################################################
        print(trace)
        # print(baseline)
        ...
        #             + delta_max9)/9
        # try just 0-5000
        # delta_max = (delta_max1 + delta_max2 + delta_max3 +delta_max9)/4

        # try just 0-1000 for a 362 frame video
        # delta_max = (delta_max1+delta_max9)/2

        delta_max = (delta_max1 + delta_max2 + delta_max9) / 3
        ...
        #for i in range(len(max_inds) - 1):
                   ...
                   # (max_inds[i+1] != max_inds[i] + 2) & \
                    #(max_inds[i+1] != max_inds[i] + 3) & \
                   # (max_inds[i+1] != max_inds[i] + 4) & \
                   # (max_inds[i+1] != max_inds[i] + 5) & \
                    #max_inds[i+1] != max_inds[i] + 6) & \
                    #(max_inds[i+1] != max_inds[i] + 7) & \
                    #(max_inds[i+1] != max_inds[i] + 4):
                #peak_inds.append(max_inds[i])
        ...
        for i in range(len(peak_inds)):
            if i < len(peak_inds)-1:
                ...
                peak_dist_lst.append(peak_dists / (30.0))  # for 30 fps
