...
directory = '/Volumes/External/Research/Jellyfish/Pulse_Counter/Output/TXT/'
all_videos = []
...
for i in all_videos:
    if '._' not in i:
        jfile = [i]
        # print(i)
        tail = os.path.basename(os.path.normpath(i))
        ...
        data_start = 0
        data_end = number_files # 20 min jump
        ...
        for jelly_numb in np.arange(number_of_jellyfish):
            ...
            print('Total pulses for' + str(tail) + ' jelly # '+ str(jelly_numb))
            print(p_numb_all)
            ...
            for item in range(len(p_t_lst)):
                ...
                df.to_csv(path_save + tail + 'J' + str(jelly_numb) + '_' + str(jumps[item]) + '_dist.csv', index=False)
                df_2.to_csv(path_save + tail + 'J' + str(jelly_numb) + '_' + str(jumps[item]) + '_peak.csv', index=False)
