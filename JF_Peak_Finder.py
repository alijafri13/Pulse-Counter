...
#path = '/Volumes/External/Research/Jellyfish/Pulse_Counter/Output/'
path = 'G:/Video_Archive/Test/Output/'
#path_for_length = '/Volumes/External/Research/Jellyfish/Pulse_Counter/Videos/'
#path_for_length = 'H:/drop_test/20210116-group1/JPGs/'
###############################################################################
###############################################################################
...
#directory = 'H:/drop_test/20210115-group1/Output/TXT/'
#all_videos = []
...
#for i in all_videos:
    #if '._' not in i:
jfile = ['G:/Video_Archive/Test/Output/TXT/20210223_1047pm_baseline_test.txt']
# print(i)
tail = '20210223_1047pm_baseline_test.txt'
tail = os.path.basename(os.path.normpath(tail))
print('Getting Peaks for ' + tail)

# length_of_data = path_for_length + i -'.txt'
#length = (path_for_length + os.path.splitext(tail)[0])
#length = os.listdir(length)  # dir is your directory path
#number_files = int(len(length))
#print(number_files)
...
lam_big = 40 # for filtering baseline
...
number_of_jellyfish = 16
#number_of_jellyfish = int(input('How many jellyfish in this video? '))
for jelly_numb in np.arange(number_of_jellyfish):
    print('Analyzing Jellyfish ' + '#' + str(jelly_numb))
    ...
    print('Total pulses for ' + str(tail.replace('.txt','')) + ' jelly # '+ str(jelly_numb))
    print(p_numb_all)
    ...
    for item in range(len(p_t_lst)):
        ...
        df.to_csv(path_save + str(tail.replace('.txt','')) + 'J' + str(jelly_numb) + '_' + str(jumps[item]) + '_dist.csv', index=False)
        df_2.to_csv(path_save + str(tail.replace('.txt','')) + 'J' + str(jelly_numb) + '_' + str(jumps[item]) + '_peak.csv', index=False)
